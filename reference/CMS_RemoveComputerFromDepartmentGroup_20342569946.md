...
# <span id="title-text"> CapaInstaller 6.7 Documentation Public : CMS_RemoveComputerFromDepartmentGroup </span>
...
:vb: CMS_RemoveComputerFromDepartmentGroup(sUnit, sGroup) As Boolean

:ps: \[bool\]CMS_RemoveComputerFromDepartmentGroup -group \<string\>
...
 If bStatus Then bStatus = CMS_RemoveComputerFromDepartmentGroup(gsUnitName, "Rome_Sales") 
```
...
``` syntaxhighlighter-pre
if (CMS_RemoveComputerToDepartmentGroup -group 'Rome_Sales') {
  $cs.Job_WriteLog("Removed", "Computer removed from department group: 'Rome_Sales'")
} 
```
