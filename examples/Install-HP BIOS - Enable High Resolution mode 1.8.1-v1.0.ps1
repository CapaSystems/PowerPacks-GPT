...
#### VARIABLES ####
###################
$global:PlaintextPassword = '' # The BIOS password you want to use. If DecryptionKey and EncryptedPassword are set, this will be ignored.

# The decryption key for the password.
$global:DecryptionKey = @(1, 103, 53, 48, 244, 116, 186, 44, 190, 231, 156, 231, 221, 70, 158, 26, 123, 243, 25, 137, 174, 15, 45, 137, 213,180,83,209,91,154,1,248)
# The encrypted password.
$global:EncryptedPassword = ''
...
function Install {
	...
	$cs.Job_WriteLog('Getting setting name')
	$SettingName = (Get-HPBIOSSettingsList | Where-Object { $_.Name -Like "Enable High Resolution*" }).Name
	$cs.Job_WriteLog("SettingName: $SettingName")

	if (Get-HPBIOSSetupPasswordIsSet) {
		$cs.Job_WriteLog('BIOS password is set')

		if ($global:DecryptionKey -and $global:EncryptedPassword) {
			$cs.Job_WriteLog('Decrypting the BIOS password to plaintext')
		...
		elseif ($global:PlaintextPassword) {
			$cs.Job_WriteLog('Using plaintext password')
		}
		else {
			$cs.Job_WriteLog('No password provided. Exiting.')
			Exit-PSScript 3329
		}

		$cs.Job_WriteLog('Setting BIOS setting.')
		Set-HPBIOSSettingValue -Name $SettingName -Value "Enable" -Password $global:PlaintextPassword
	}
 else {
		$cs.Job_WriteLog('Setting BIOS setting.')
		Set-HPBIOSSettingValue -Name $SettingName -Value "Enable"
	}
}
