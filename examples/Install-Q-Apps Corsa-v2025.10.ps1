...
function InstallQplant {
    ...
    $Destination = Join-Path $global:gsProgramFilesx86 'QPlant'

    $cs.File_CopyTree($Source, $Destination)

    $Splat = @{
        LiteralPath     = Join-Path $global:gsCommonPrograms "Q-Apps Corsa" 'TAI-Plataforma.lnk'
        TargetPath      = Join-Path $Destination 'TAIPlataforma.exe'
        WorkingDirectory= $Destination
        RunAsAdmin      = $true
    }
    Desktop_CreateShortCut @Splat
}
...
function InstallOracle {
    ...
    if ($cs.File_ExistFile($ExePath)){
        $cs.Job_WriteLog("Oracle installer found: $ExePath")
        $cs.Job_WriteLog("Take this as Oracle is already installed, skipping installation.")
        return
    }
...
function InstallTaiQplant {
    $cs.Log_SectionHeader('InstallTaiQplant', 'o')

    $Source = Join-Path $global:Packageroot 'kit' "6 - Tai Qplant"
    $Destination = "C:\Tai\Qplant"

    $cs.File_CopyTree($Source, $Destination)

    $Splat = @{
        TargetPath       = Join-Path $Destination 'Abrir_Qplant.bat'
        LiteralPath      = Join-Path $global:gsCommonPrograms "Q-Apps Corsa" 'Tai Qplant.lnk'
        WorkingDirectory = $Destination
    }
    Desktop_CreateShortCut @Splat

    $Files = @(
        [PSCustomObject]@{ FileName = '\Actualizar_Qplant_CAR.bat'; ShortCutName = "Actualizar_Qplant_CAR.lnk"}
        , [PSCustomObject]@{ FileName = '\Actualizar_Qplant_SPA.bat'; ShortCutName = 'Actualizar_Qplant_SPA.lnk' }
        , [PSCustomObject]@{ FileName = '\Actualizar_Qplant_OLO.bat'; ShortCutName = 'Actualizar_Qplant_OLO.lnk' }
    )
    $DestinationFolder = Join-Path $global:gsProgramFilesx86 'QPlant'

    foreach ($File in $Files) {
        $SourceFile = Join-Path $Destination $File.FileName
        $DestinationFile = Join-Path $DestinationFolder $File.FileName
        $cs.File_CopyFile($SourceFile, $DestinationFile)

        $Splat = @{
            TargetPath       = $DestinationFile
            LiteralPath      = Join-Path $global:gsCommonPrograms "Q-Apps Corsa" $File.ShortCutName
...
function InstallQPlantManager {
    $cs.Log_SectionHeader('InstallQPlantManager', 'o')

    $Source = Join-Path $global:Packageroot 'kit' "7 - Tai QPlant Manager"
    $Destination = "C:\Tai\Framework"

    $cs.File_CopyTree($Source, $Destination)

    $ShortcutSource = Join-Path $Destination 'Q-Plant Enterprise Manager.lnk'
    $ShortcutDestination = Join-Path $global:gsCommonPrograms "Q-Apps Corsa" 'Q-Plant Enterprise Manager.lnk'
    $cs.File_CopyFile($ShortcutSource, $ShortcutDestination)

    # Set shortcut to run program as administrator.
    $fileBytes = [System.IO.File]::ReadAllBytes($ShortcutDestination)
    $fileBytes[21] = $fileBytes[21] -bor 32
    [System.IO.File]::WriteAllBytes($ShortcutDestination, $fileBytes)
}
