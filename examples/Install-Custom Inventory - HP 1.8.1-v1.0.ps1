...
function ImportModules {
  $cs.Log_SectionHeader("ImportModules", 'o')

  $Path = Join-Path $global:Packageroot 'kit' 'HPCMSL module'
  $Folders = Get-ChildItem -Path $Path -Directory

  $cs.Job_WriteLog("Adding $Path to Env:PSModulePath")
  $Env:PSModulePath = $Env:PSModulePath+";$Path"

  foreach ($Folder in $Folders) {
    $cs.Job_WriteLog("Importing $($Folder.Name)")
    Import-Module $Folder.Name -Force
  }
}

function BIOSPassword {
  $cs.Log_SectionHeader("BIOSPassword", 'o')

  $Splat = @{
    category  = "HP"
    entry     = "BIOS Setup Password is set"
    value     = Get-HPBIOSSetupPasswordIsSet
    valuetype = "S"
  }
  CMS_AddCustomInventory @Splat
}
