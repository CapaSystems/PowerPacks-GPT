...
function Install {
  ...
  $Command = $null
  if ($cs.File_ExistFile("c:\Program Files\Microsoft Visual Studio\2022\Professional\Common7\IDE\VSIXInstaller.exe")) {
    $Command = "c:\Program Files\Microsoft Visual Studio\2022\Professional\Common7\IDE\VSIXInstaller.exe"
  }
  if ($cs.File_ExistFile("c:\Program Files\Microsoft Visual Studio\2022\Enterprise\Common7\IDE\VSIXInstaller.exe")) {
    $Command = "c:\Program Files\Microsoft Visual Studio\2022\Enterprise\Common7\IDE\VSIXInstaller.exe"
  }

  if ($null -eq $Command) {
    Exit-PSScript 1 -exitmessage "No VSIXInstaller.exe found"
  } else {
    $cs.Job_WriteLog("VSIXInstaller.exe path: $Command")
  }

  $VsixPath = Join-Path $global:Packageroot "kit" "Microsoft.DataTools.ReportingServices.vsix"
  $VsixPathTemp = "C:\Temp\Microsoft.DataTools.ReportingServices.vsix"
  $Arguments = "/quiet /force `"$VsixPathTemp`""

  $cs.File_CopyFile($VsixPath, $VsixPathTemp, $true)

  $RetValue = $cs.Shell_Execute($Command, $Arguments)

  $cs.File_DelFile($VsixPathTemp, $false)
