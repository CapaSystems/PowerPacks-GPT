function PSAppDeployToolkit {
  ...
  try {
    ...
    if (-not (Test-Path -Path $ConfigFile)) {
      ...
      $DefaultLogPath = Join-Path $env:SystemRoot 'Logs' 'Software'
      $Paths.Add($DefaultLogPath)
      $cs.Job_WriteLog("$LogPreTag Using default log path: $DefaultLogPath")
    } else {
    ...
    if ($cs.File_ExistFile($ProcessPath) -eq $false) {
      $ProcessPath = Join-Path $Packageroot 'Deploy-Application.exe'

      if ($cs.File_ExistFile($ProcessPath) -eq $false) {
        $cs.Job_WriteLog("$LogPreTag Neither Invoke-AppDeployToolkit.exe nor Deploy-Application.exe were found in the package root: $Packageroot")
        throw "Neither Invoke-AppDeployToolkit.exe nor Deploy-Application.exe were found in the package root: $Packageroot"
      }
    }

    $cs.Job_WriteLog("$LogPreTag ProcessPath: $ProcessPath")

    [version]$PSADTVersion = (Get-Item $ProcessPath).VersionInfo.FileVersion
    $cs.Job_WriteLog("$LogPreTag PSAppDeployToolkit Version: $PSADTVersion")

    $ProcessArguments = ''
    if ($Architecture -ne 'x64') {
      $ProcessArguments = "$ProcessArguments /32"
    }
    if ($UsePowerShellCore -and $PSADTVersion -ge [version]'4.1') {
      $ProcessArguments = "$ProcessArguments /Core"
    }
